export function initProductDetail() {
  ...
  // Настоящее поле формы add-to-cart (woocommerce/single-product/add-to-cart/simple.php) —
  // то, что реально уходит на сервер; counterValue — только отображение
  const counterInput = root.querySelector("[data-counter-input]");
  ...
  const renderQuantity = () => {
    ...
    if (counterInput) counterInput.value = String(quantity);
  };
  ...
  // Кнопка "Add to cart" — обычный submit настоящей формы WooCommerce,
  // отдельный обработчик клика ей больше не нужен
}
